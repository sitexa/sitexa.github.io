...
Keytool是一个Java数据证书的管理工具，这个工具一般在```JDK\jre\bin\security\```目录下 。所有的数字证书是以一条一条(采用别名区别)的形式存入证书库的，证书库中的每个证书包含该条证书的私钥，公钥和对应的数字证书的信息。
...
``` 
keytool -genkeypair -alias "tomcat" -keyalg "RSA" -keystore "${HOME}/mykeystore.keystore"  -dname "CN=localhost, OU=localhost, O=localhost, L=SH, ST=SH, C=CN" -keypass "123456" -storepass -validity 180
```
...
cacerts证书文件(The cacerts Certificates File),该证书文件存在于```java.home\jre\lib\security```目录下，是Java系统的CA证书仓库。
